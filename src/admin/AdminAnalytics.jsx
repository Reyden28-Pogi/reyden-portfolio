  ...
  return (
    <div className="analytics">
      <div className="analytics__stats">
        <StatCard label="Total Visits" value={loading ? "..." : stats.total} icon="👁" />
        <StatCard label="Today" value={loading ? "..." : stats.today} icon="📅" />
        <StatCard label="This Week" value={loading ? "..." : stats.week} icon="📆" />
        <StatCard label="Days Tracked" value={loading ? "..." : stats.history.length} icon="📊" />
      </div>

      <div className="admin-card">
        <p className="admin-card__title">Last 14 Days — Daily Visits ☁️ Live from Supabase</p>
        {loading ? (
          <p style={{ fontSize: "0.82rem", color: "var(--text-muted)" }}>Loading analytics...</p>
        ) : history.length === 0 ? (
          <p className="analytics__empty">
            No visit data yet. Visits are tracked when someone opens your portfolio.
          </p>
        ) : (
